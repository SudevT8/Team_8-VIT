...
class EmployeeService {
  ...
  create(name: string, department: string, salary: number): Employee {
    const emp: Employee = { id: this.nextId++, name, department, salary };
...
6. Exit`);
    ...
    if (choice === "1") {
      ...
      if (list.length === 0) {
        ...
        console.log("\n--- All Employees ---");
        list.forEach((e) =>
          console.log(`ID: ${e.id} | Name: ${e.name} | Dept: ${e.department} | Salary: ₹${e.salary}`)
        );
      }
      ...
      const updated = service.update(id, name.trim(), dept.trim(), salary);
      console.log(updated ? "✅ Employee updated." : "❌ Employee not found.");

    } else if (choice === "5") {
      const id = parseInt(await ask("Enter Employee ID to delete: "));
      const deleted = service.delete(id);
      console.log(deleted ? "✅ Employee deleted." : "❌ Employee not found.");
